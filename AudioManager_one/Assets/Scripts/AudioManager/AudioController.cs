...
namespace UnityCore
{
    namespace Audio
    {
        public class AudioController : MonoBehaviour
        {
            ...
            //** Relationship between audio types (key) and audio tracks (value) **//
            private Hashtable m_AudioTable;

            //** Relationship between audio types (key) and jobs (value) **//
            ...
            IEnumerator RunAudioJob(AudioJob _job)
            {
                ...
                if (_job.fade)
                {
                    float _initialValue = _job.action == AudioAction.START || _job.action == AudioAction.RESTART ? 0.0f : 1.0f;
                    float _target = _initialValue == 0.0f ? 1.0f : 0.0f;
                    float _duration = 1.0f;
                    float _timer = 0.0f;

                    while (_timer <= _duration)
                    {
                        _track.source.volume = Mathf.Lerp(_initialValue, _target, _timer / _duration);
            ...
            public AudioClip GetAudioClipFromTrack(AudioType _type, AudioTrack _track)
            {
            ...
            //** In case audio controller is deleted **//
            ...
            private void PopulateAudioTable()
            {
                foreach (AudioTrack _track in tracks)
                {
                    foreach (AudioObject _obj in _track.audio)
                    {
                        ...
                        {
                            LogWarning("Trying to register audio [" + _obj.type + "] that has already been registered");
